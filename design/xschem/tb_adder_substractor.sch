...
C {SUBTRACTOR/8-bit_adder_subtractor.sym} -20 10 0 0 {name=x1}
...
"
spice_ignore=false}
C {devices/code_shown.sym} 200 -60 0 0 {name=s1 only_toplevel=false value="

* Apply test cases using DC sources and pulses for inputs
* Test Case 1: A = 8'b00000001, B = 8'b00000001, K = 0 (Addition)
V_K K 0 DC 0
V_A0 A0 0 PULSE(0 1 0 1n 1n 20n 40n)
V_B0 B0 0 PULSE(0 1 0 1n 1n 20n 40n)
V_A1 A1 0 DC 0
V_B1 B1 0 DC 0
V_A2 A2 0 DC 0
V_B2 B2 0 DC 0
...
save all
tran 1n 100n
plot S0 S1 S2 S3 S4 S5 S6 S7 title 'Outputs for A=1, B=1, K=0'
.endc
"}
